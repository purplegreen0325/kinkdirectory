...
function getPositionLabel(position: string): string {
  if (position === 'general') return t('app.general')
  if (position === 'as_dom') return t('app.as_dom')
  if (position === 'as_sub') return t('app.as_sub')
  if (position === 'for_dom') return t('app.for_dom')
  if (position === 'for_sub') return t('app.for_sub')
  return ''
}

// Get the position to display in the left column
const leftColumnPosition = computed(() => {
  if (props.kink.format === 'general') return 'general'
...
const isPositionApplicable = (position: string) => {
...
<template>
  <KinkRowLayout :isLastItem="isLastItem">
    <template #label>
    ...
    <template #choices>
        ...
        <td class="text-center py-2 px-0 sm:px-2 min-w-[40px] sm:min-w-[50px]">
          <div class="flex justify-center items-center">
            <KinkChoice 
              ...
              :onClick="(value) => handleClick('general', value)" 
              :kinkName="t(labelPath)"
      ...
      <template v-else-if="kink.format === 'role_specific'">
        ...
        <KinkColumn 
          v-if="showLeftColumn"
          :isEmpty="!isPositionApplicable(leftColumnPosition)"
          :position="leftColumnPosition"
          :value="getKinkChoice(categoryId, kink.id, leftColumnPosition)"
          :kinkName="t(labelPath)"
          :positionLabel="getPositionLabel(leftColumnPosition)"
          :tooltip="t(tooltipPath)"
          :onClick="(value) => handleClick(leftColumnPosition, value)"
        />
        
        <!-- Second Column (For your dom/sub) - only if we need right column -->
        <KinkColumn 
          v-if="needsRightColumn"
          :isEmpty="!isPositionApplicable(rightColumnPosition)"
          :position="rightColumnPosition"
          :value="getKinkChoice(categoryId, kink.id, rightColumnPosition)"
          :kinkName="t(labelPath)"
          :positionLabel="getPositionLabel(rightColumnPosition)"
          :tooltip="t(tooltipPath)"
          :onClick="(value) => handleClick(rightColumnPosition, value)"
        />
      </template>
    </template>
  </KinkRowLayout>
